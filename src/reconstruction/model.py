...
class UNetInpainting(nn.Module):
    def __init__(self):
        ...
        self.dec5 = nn.Sequential(nn.Conv2d(512+512, 512, 3, padding=1), nn.ReLU())
    ...
    def forward(self, y, mask):
        ...
        e4 = F.relu(e4)
        e5, _  = self.enc5(e4, m4)
        e5 = F.relu(e5)

        d = F.interpolate(e5, scale_factor=2, mode='nearest')
        d = self.dec5(torch.cat([d, e4], dim=1))

        d = F.interpolate(d, scale_factor=2, mode='nearest')
